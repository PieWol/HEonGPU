#!/usr/bin/env python3
"""
Benchmark HE minimum for varying N, mimicking the experimental setup of:
  "Efficient Ranking, Order Statistics, and Sorting under CKKS"
  Mazzone et al., USENIX Security 2025

Usage:
    python3 benchmark_minimum.py [--n-values N1 N2 ...] [--runs R] [--output FILE]

    # Quick test:
    python3 benchmark_minimum.py --n-values 8 16 32 --runs 1

    # Full paper-equivalent single-ciphertext sweep (default):
    python3 benchmark_minimum.py

    # Custom N set, 5 runs each:
    python3 benchmark_minimum.py --n-values 8 16 32 64 128 --runs 5
"""
...
BINARY_DEFAULT = _SCRIPT_DIR.parent / "build/bin/examples/ranking_and_sorting/19_ckks_minimum"
...
def run_once(binary: Path, n: int) -> dict | None:
    ...
    # Format: BENCH: N=8 ctx_ms=... keygen_ms=... min_ms=... gpu_keys_mib=... gpu_min_mib=... gpu_peak_mib=...
...
def benchmark_n(binary: Path, n: int, runs: int) -> dict | None:
    ...
    for run_idx in range(runs):
        ...
        total_s = (data.get('ctx_ms', 0) + data.get('keygen_ms', 0) + data.get('min_ms', 0)) / 1000
        print(f"ctx_ms={data.get('ctx_ms', 0):.1f}  "
              f"min_ms={data.get('min_ms', 0):.1f}  "
              f"total={total_s:.2f}s  "
              f"gpu_keys={data.get('gpu_keys_mib', '?')}MiB  "
              f"gpu_min={data.get('gpu_min_mib', '?')}MiB  "
    ...
    avg["min_s"]    = avg["min_ms"] / 1000.0
    avg["keygen_s"] = avg["keygen_ms"] / 1000.0
    avg["total_s"]  = (avg["ctx_ms"] + avg["keygen_ms"] + avg["min_ms"]) / 1000.0
    return avg


def print_table(results: list[dict]) -> None:
    print("\n" + "=" * 125)
    print(f"{'N':>6}  {'ctx_ms':>10}  {'keygen_ms':>12}  {'min_ms':>12}  "
          f"{'min_s':>10}  {'total_s':>10}  {'keys_MiB':>10}  {'min_MiB':>10}  {'peak_MiB':>10}")
    print("-" * 125)
    for r in results:
        cx  = r.get("ctx_ms", 0)
        ...
        gm  = r.get("gpu_min_mib", float("nan"))
        gp  = r.get("gpu_peak_mib", float("nan"))
        print(f"{r['n']:>6}  {cx:>10.1f}  {ks:>12.1f}  {ms:>12.1f}  "
              f"{ms/1000:>10.3f}  {ts:>10.1f}  {gk:>10.0f}  {gm:>10.0f}  {gp:>10.0f}")
    print("=" * 125)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark HE minimum for varying N",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "--binary", type=Path, default=BINARY_DEFAULT,
        help="Path to 19_ckks_minimum binary"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", type=Path,
        default=_SCRIPT_DIR / "minimum_benchmark_results.csv",
        help="CSV output file (default: minimum_benchmark_results.csv)"
    )
    args = parser.parse_args()

    if not args.binary.exists():
        print(f"Binary not found: {args.binary}")
        print("Build first with: cmake --build build --target 19_ckks_minimum")
        sys.exit(1)
    ...
    for n in n_values:
        ...
        if result:
            ...
            print(f"  → avg min: {min_ms:.1f} ms ({min_ms/1000:.3f} s)")
        else:
