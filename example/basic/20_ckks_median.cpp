 ...
 *
 * Homomorphic median (N/2-th order statistic) via Algorithm 5 from:
 *   "Efficient Ranking, Order Statistics, and Sorting under CKKS"
 *   Mazzone, Everts, Hahn, Peter — USENIX Security 2025
 *
 * Runs the full sorting pipeline (homomorphicSortFG) and reads slot (N/2)*N,
 * which holds the (N/2+1)-th order statistic = upper median of the input vector.
 *
 * For even N: returns the upper median (the larger of the two middle values).
 * For odd N:  returns the true median.
 *
 * Output: slot (N/2)*N of the decoded result = normalized median.
...
int main(int argc, char* argv[])
{
    ...
    // Compute expected median: upper median for even N (index N/2 in sorted order)
    std::vector<double> input_sorted = input;
    std::sort(input_sorted.begin(), input_sorted.end());
    int k_med = N / 2;  // 0-indexed: upper median
    double expected_median = input_sorted[k_med];
    ...
    // Decrypt and extract slot k_med*N = (k_med+1)-th order statistic
    ...
    double range  = hi - lo;
    double he_median = raw[k_med * N] * range + lo;  // slot k_med*N
